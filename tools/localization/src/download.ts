 ...
 *
 * @description
 * buildAndDownload() will build zip file on Crowdin server and download it as ankidroiddocs.zip file.
 * It's expected to be called through 'yarn start download'.
 *
 * extractZip() will extract downloaded ankidroiddocs.zip file.
        ...
        const buildProgress = setInterval(async () => {
            ...
            if (progress.data.progress === 100 && progress.data.status === "finished") {
                ...
                axios({
                    ...
                    response.data.pipe(fs.createWriteStream("ankidroiddocs.zip"));
                });
            }
        }, 10_000);
    } catch (error) {
